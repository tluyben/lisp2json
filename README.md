...
`lisp2json` is a library and CLI tool that allows you to convert Lisp expressions to JSON and vice versa. It's perfect for developers working with Lisp who need to interface with JSON-based systems or APIs.

This project provides implementations in both Go and TypeScript.
...
// Convert Lisp to JSON
const jsonStr = lisp2JSON('(print "Hello, World!")');

// Convert JSON to Lisp
const lispStr = json2Lisp(
  '{"cmd":"print","args":[{"lit":"Hello, World!","type":"string"}]}'
);
```

### As a CLI tool (Go version)
...
### As a CLI tool (TypeScript version)

```bash
# Convert Lisp file to JSON
npx lisp2json-ts --lisp example.lsp

# Convert JSON file to Lisp
npx lisp2json-ts --json example.json
```
...
To run the Go tests, use the following command:
...
To run the TypeScript tests, use the following command:
...
To run all tests (both Go and TypeScript), use:
